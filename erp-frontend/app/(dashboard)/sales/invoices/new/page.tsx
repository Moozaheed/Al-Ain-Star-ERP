...
import type { Customer, LineItemDraft, PaymentMode, SalesChannel } from "@/types/sales";
...
export default function NewInvoicePage() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: object) => api.post("/sales/invoices", payload),
  ...
  function handleSubmit() {
    ...
    if (!user?.branch && !user?.id) { setError("No branch assigned to your account."); return; }

    createMutation.mutate({
      branch_id:    user?.branch?.id ?? 1,
      customer_id:  selectedCustomer?.id ?? null,
      customer_name: selectedCustomer?.name ?? (walkInName || "CASH CUSTOMER"),
      channel,
    ...
    <div className="flex flex-col gap-6 pb-10">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">Create a sales invoice with VAT</p>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-6 lg:col-span-2">
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                <Input label="LPO Number" value={lpoNumber} onChange={(e) => setLpoNumber(e.target.value)} placeholder="Customer PO ref" />
                <Input label="Reference No" value={refNumber} onChange={(e) => setRefNumber(e.target.value)} placeholder="Internal ref" />
                {paymentMode === "credit" && (
                ...
                <div className="flex flex-col gap-3">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Search customer by name, phone, TRN..."
                      ...
                      <div className="absolute left-0 right-0 top-full z-20 mt-1 rounded-lg border border-gray-200 bg-white shadow-lg">
                        {customers.map((c) => (
                          <button key={c.id} className="flex w-full items-center gap-3 px-4 py-2.5 text-left hover:bg-gray-50"
                            ...
                            <Badge variant="neutral" className="ml-auto">{c.type}</Badge>
                          </button>
                        ))}
                      </div>
                    )}
                  </div>
                  <p className="text-xs text-gray-400">Or enter a walk-in name:</p>
                  <Input value={walkInName} onChange={(e) => setWalkInName(e.target.value)}
                    placeholder="Walk-in customer name (leave blank for CASH CUSTOMER)" />
                </div>
              )}
            </CardContent>
          </Card>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {items.map((item, idx) => {
                      ...
                      return (
                        <tr key={idx}>
                          {/* Part — set by the search bar above, not editable in place */}
        ...
        <div className="flex flex-col gap-4">
          <Card>
            ...
            <CardContent className="flex flex-col gap-3">
              ...
              <SummaryRow label={`VAT (${VAT_RATE}%)`} value={formatAed(vatAmount)} />
              ...
              <div className="mt-1 rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-500">
                ...
                {["cash", "card", "bank_transfer"].includes(paymentMode) && (
                  <p className="mt-0.5 text-green-700">Invoice will be marked as Paid</p>
                )}
                {["credit", "cheque"].includes(paymentMode) && (
                  <p className="mt-0.5 text-amber-700">Invoice will be marked as Confirmed (payment pending)</p>
                )}
              </div>
