    ...
    <div className="flex flex-col gap-4">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Invoice</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-12 text-center text-sm text-gray-400">Nothing outstanding — all invoices are settled.</td></tr>
              ) : rows.map((r) => (
      ...
      {paying && (
        <RecordPaymentModal
          ...
          onSaved={() => { setPaying(null); qc.invalidateQueries({ queryKey: ["accounting-receivables"] }); }}
        />
      )}
    </div>
  );
...
function RecordPaymentModal({ row, onClose, onSaved }: { row: AgingRow; onClose: () => void; onSaved: () => void }) {
  ...
  const [paymentMode, setPaymentMode] = useState<"cash" | "card" | "bank_transfer" | "cheque">("cash");
  ...
  const mutation = useMutation({
    mutationFn: () => api.post("/sales/payments", {
      invoice_id: row.id,
        ...
        <div className="flex flex-col gap-4 px-6 py-5">
          ...
          {paymentMode === "cheque" ? (
            ...
            <Field label="Reference">
              <input value={reference} onChange={(e) => setReference(e.target.value)} className={inputCls} placeholder="Transaction / receipt ref" />
            </Field>
          )}
